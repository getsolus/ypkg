...
class PackageSanity:
    ...
    def is_name_valid(name):
        ...
        name = str(name.strip())
        if len(name) < 1:
            console_ui.emit_error("YAML:name", "Package name cannot be empty")
            return False

        if " " in name:
            console_ui.emit_error(
                "YAML:name", "Package names cannot contain whitespace"
            )
            return False
        illegal = set()
        permitted = ["-", "_", "+", "."]
        ...
        console_ui.emit_error(
            "YAML:name",
            "Illegal characters in package name '{}' : {}".format(
                name, ", ".join(illegal)
            ),
        )
        return False
    ...
    def is_version_valid(version):
        ...
        try:
            ...
            console_ui.emit_error("YAML", "Invalid version: {}".format(version))
            return False
        return True
...
class YpkgSpec:
    ...
    def add_replace(self, key, val):
        ...
        if val in self.replaces and val != key:
            console_ui.emit_warning("YAML", "Duplicate replace: {}".format(val))
            return
    ...
    def add_conflict(self, key, val):
        ...
        if val in self.conflicts:
            console_ui.emit_warning("YAML", "Duplicate conflict: {}".format(val))
            return
    ...
    def init_defaults(self):
        ...
        if "32bit-devel" not in self.summaries:
            self.add_summary(
                "32bit-devel", "Development files for 32-bit {}".format(name)
            )
        ...
        if "32bit-dbginfo" not in self.summaries:
            self.add_summary(
                "32bit-dbginfo", "32-bit debug symbols for {}".format(name)
            )
        ...
        if self.pkg_optimize:
            if (
                "thin-lto"
                or "icf-safe"
                or "icf-all" in self.pkg_optimize
                and not self.pkg_clang
            ):
    ...
    def load_from_path(self, path):
        ...
        if filename != "package.yml":
            console_ui.emit_warning(
                "Unnecessarily Anal Warning", "File is not named package.yml"
            )
        ...
        for tk_set in sets:
            for token in list(tk_set.keys()):
                ...
                if not hasattr(self, instance_name):
                    console_ui.emit_error(
                        "YAML:{}".format(token), "Internal error for unknown token"
                    )
                    return False
    ...
    def load_component(self):
        ...
        for i in ["component.xml", "../component.xml"]:
            ...
            try:
                comp.read(fpath)
                console_ui.emit_error(
                    "Component", "Using legacy component.xml: {}".format(fpath)
                )
