...
from .util import console_ui, remove_prefix, EMUL32PC
...
def execute_step(context, step, step_n, work_dir):
    ...
    if context.gen_pgo and context.spec.pkg_clang:
        profileFile = os.path.join(context.get_pgo_dir(), "default-%m.profraw")
        ...
        profileFile = os.path.join(context.get_pgo_dir(), "default.profdata")
...
def build_package(filename, outputDir, buildDir=None):
    ...
    # Dummy content
    console_ui.emit_info("Info", f"Building {spec.pkg_name}-{spec.pkg_version}")
        ...
        for step, context in run:
            ...
            if step == "setup":
                ...
                if spec.pkg_extract:
                    ...
                    console_ui.emit_info("Source", "Extracting source")
            ...
            console_ui.emit_info("Build", f"Running step: {step}")
            ...
            if execute_step(context, r_step, step, work_dir):
                ...
                console_ui.emit_success(
                    "Build",
                    f"{step} successful ({timedelta(seconds=end_time - start_time)})",
                )
                continue
            console_ui.emit_error("Build", f"{step} failed for {spec.pkg_name}")
            sys.exit(1)
    ...
    if context.spec.pkg_mancompress:
        man_dirs = [
            f"{ctx.get_install_dir()}/usr/share/man",
            f"{ctx.get_install_dir()}/usr/man",
        ]
        for dir in man_dirs:
            if not os.path.exists(dir):
                continue

            console_ui.emit_info("Man", f"Compressing manpages in '{dir}'...")
            try:
                ...
                console_ui.emit_warning(
                    "Man", f"Failed to compress man pages in '{dir}'"
                )
                print(e)

        # Now try to compress any info pages
        info_dir = f"{ctx.get_install_dir()}/usr/share/info"
    ...
    for root, dirs, files in os.walk(idir):
        ...
        if len(dirs) == 0 and len(files) == 0:
            console_ui.emit_warning(
                "Package",
                f"Including empty directory: {remove_prefix(root, idir)}",
            )
    ...
    if exaResults is None:
        console_ui.emit_error("Package", "Failed to correctly examine all packages.")
        sys.exit(1)

    deps = DependencyResolver()
    if not deps.compute_for_packages(ctx, gene, exaResults):
        console_ui.emit_error("Dependencies", "Failed to compute all dependencies")
        sys.exit(1)
    ...
    if len(gene.packages) == 0:
        ...
        print(
            (
                "Ensure your files end up in $installdir. Did you mean to "
                f"use %make_install?\n\nPlease see the help center: {w}"
            )
        )
        sys.exit(1)
    ...
    for package in sorted(gene.packages):
        ...
        if len(files) == 0:
            console_ui.emit_info("Package", f"Skipping empty package: {package}")
            continue
    ...
    for pkg in spec.patterns:
        ...
        console_ui.emit_warning(
            f"Package:{pkg}", f"Did not produce {nm} by any pattern"
        )
